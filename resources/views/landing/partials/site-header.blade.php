        ...
        <nav class="mx-auto flex max-w-7xl items-center justify-between px-5 py-3">
            ...
            <div class="flex items-center gap-2">
                @if(data_get($headerContent, 'demo_label') && data_get($headerContent, 'demo_url'))
                    <a href="{{ $marketingUrl(data_get($headerContent, 'demo_url')) }}" class="hidden rounded-lg border border-zinc-300 px-4 py-2 text-sm font-black md:inline-flex">{{ data_get($headerContent, 'demo_label') }}</a>
                @endif
