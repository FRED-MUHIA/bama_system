...
class Invoice extends Model
{
    ...
    public function ensurePublicToken(): string
    {
        ...
        if (! $this->exists) {
            throw new \LogicException('Save the invoice before generating its public link.');
        }

        // Target this already-resolved invoice only. A conditional update keeps
        // concurrent requests from replacing a token generated by another request.
        $this->newModelQuery()->whereKey($this->getKey())
            ->where(function (Builder $query) {
                $query->whereNull('public_token')->orWhereRaw("TRIM(public_token) = ''");
            })->toBase()->update(['public_token' => Str::random(48)]);

        $this->public_token = $this->newModelQuery()->whereKey($this->getKey())
            ->firstOrFail(['public_token'])->public_token;
        $this->syncOriginalAttribute('public_token');
